import { UpdatePasswordData, UserLoginData, UserRegistrationData, UserUpdateData } from "../../types/userTypes";
...
export const validateUpdatePassword = (userInfo: UpdatePasswordData): { isValid: boolean; error?: string } => {
	const requiredFields = ["password", "email", "confirm_password", "code"]
	const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
	
	for (const field of requiredFields) {
		if(!userInfo[field as keyof UpdatePasswordData]) {
			return {isValid: false, error:`${field} is required`}
		}
	}
	if (!emailRegex.test((userInfo.email))) {
